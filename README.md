...
- **Grid-locked beats + mic-free mode** — once a track is profiled, its beats are a known timetable: the engine computes every beat from BPM + beat phase + Spotify playback position and fires each light command `beat_lead_ms` early, so BLE latency lands the flash *on* the beat instead of after it. With `reactive.mic_free` enabled, profiled tracks are driven entirely from the profile — no microphone needed; the mic's only remaining job is analyzing tracks it hasn't heard yet. `reactive.beat_source` picks the timing source (`auto`/`mic`/`grid`).
